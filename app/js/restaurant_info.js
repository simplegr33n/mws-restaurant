...
const addReview = (event) => {
  ...
  if (nameValue == null || nameValue == '' || ratingValue == null || ratingValue == '' || commentsValue == null || commentsValue == '') {
    Toast.showToast('Please include your name, comment, and rating!');
    return;
  }
...
const resetReviewForm = (nameField, ratingField, commentsField) => {
  nameField.value = '';
  ratingField.value = '';
  commentsField.textContent = "What'd you think about this place..?";
};
...
const syncReviewsWithServer = () => {
  Promise.all(unsuncReviews.map(review => {
    DBHelper.postReviewToServer(review);
  })).then(_ => {
    Toast.showToast('Uploading reviews successful!');
    unsuncReviews.length = 0;
  }).catch(_ => {
    unsuncReviews.length = 0;
  });
};
