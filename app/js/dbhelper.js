...
export let unsuncReviews = [];
export let unsuncRestaurants = [];
...
export class DBHelper {
  ...
  /**
   * Fetch all restaurants via either cache first or network first strategies with error handling.
   */
  static fetchRestaurants(callback) {
    ...
    if (navigator.onLine) {
      fetch(DBHelper.RESTAURANTS_URL)
        ...
        .then(restaurants => {
          if (!restaurants || (restaurants && restaurants.length === 0)) throw new Error('No restaurants found!');
      ...
      DBHelper.getRestaurantsFromDb(dbPromise)
        .then((restaurants) => {
          if (restaurants && restaurants.length > 0) {
            ...
            fetch(DBHelper.RESTAURANTS_URL)
              .then(response => {
                if (!response) throw new Error("Server didn't respond!");
    ...
    DBHelper.getRestaurantsFromDb(dbPromise)
      .then(restaurants => {
        ...
        if (navigator.onLine) {
          ...
          unsuncRestaurants.push(restaurant);
        }
      });
  }
  ...
  static webPImageUrlForRestaurant(restaurant) {
    return (`dist/img/webp/${restaurant.photograph || restaurant.id}.webp`);
  }

  static jpegImageUrlForRestaurant(restaurant) {
    return (`dist/img/${restaurant.photograph || restaurant.id}.jpg`);
  }
