...
const createRestaurantHTML = (restaurant) => {
  ...
  detailsBtn.className = 'card-action-view';
  detailsBtn.innerHTML = 'View Details';
  detailsBtn.href = DBHelper.urlForRestaurant(restaurant);
  
  detailsBtn.setAttribute('role', 'button');
  detailsBtn.setAttribute('aria-label', `view details of ${restaurant.name} restaurant`);

  const favBtn = document.createElement('button');
  favBtn.className = 'card-action-favorite';
  favBtn.dataset.id = restaurant.id;
  favBtn.dataset.favorite = (restaurant.is_favorite == undefined || restaurant.is_favorite == 'undefined' || restaurant.is_favorite === false || restaurant.is_favorite === 'false') ? false : true;
  favBtn.setAttribute('aria-label', `mark ${restaurant.name} restaurant as favorite`);
  if (favBtn.dataset.favorite === 'true') {
    favBtn.innerHTML = '&#10084;';
  } else if (favBtn.dataset.favorite === 'false') {
    favBtn.innerHTML = '&#9785;';
  }
  favBtn.addEventListener('click', toggleFavoriteRestaurant);

  const actionButtonList = document.createElement('section');
  actionButtonList.className = 'card-action';
  actionButtonList.append(favBtn);
  actionButtonList.append(detailsBtn);
...
const syncFavoriteRestaurantsWithServer = () => {
  Promise.all(unsuncRestaurants.map(restaurant => {
    DBHelper.updateFavoriteToServer(restaurant);
  })).then(_ => {
    Toast.showToast('Favorites updated successfully!');
    unsuncRestaurants.length = 0;
  }).catch(_ => {
    unsuncRestaurants.length = 0;
  });
};
...
const isOnline = (event) => {
  Toast.showToast('App is online, updating...');
...
const isOffline = (event) => {
  Toast.showToast('App in offline-mode.');
};
